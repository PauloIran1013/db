...
SELECT  c.nome AS municipio,SUM(s.publico) AS total_publico
FROM  sessao s
...
SELECT c. nomeFantasia AS cinema_nome,SUM(s.publico) AS total_publico
FROM cinema c
JOIN sala sa ON c.id = sa.cinema_id
JOIN sessao s ON sa.id = s.sala_id
GROUP BY c.id, c.nomeFantasia;


-- 3. Apuração do público por sessão de cada cinema;
SELECT c.nomeFantasia AS cinema_nome,s.id, SUM(s.publico) AS total_publico
FROM cinema c
JOIN sala sa ON c.id = sa.cinema_id
JOIN sessao s ON sa.id = s.id
GROUP BY c.id, s.id;  
...
SELECT  DISTINCT c.nomeFantasia AS cinema_nome
FROM ator a
JOIN filme f ON a.id = f.id  
JOIN sessao s ON f.id = s.id
JOIN sala sa ON s.id = sa.id
JOIN  cinema c ON sa.id = c.id
WHERE  a.nome = 'Martin Scorsese';  

-- 5. Em quais cinemas está sendo exibido um determinado gênero de filme;
SELECT DISTINCT c.nomeFantasia AS cinema_nome
FROM genero g
JOIN filme f ON g.id = f.id  -- ajuste conforme a relação correta
JOIN sessao s ON f.id = s.id
JOIN sala sa ON s.id = sa.id
JOIN cinema c ON sa.id = c.id
